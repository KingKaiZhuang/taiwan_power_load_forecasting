...
with gr.Blocks(title="基於機器學習之台灣電力負載預測系統") as demo:
    ...
    with gr.Tabs():
        # 分頁 1: 預測儀表板
        view_radio, month_filter, plot_output, plot_peak_output, table_output, seq_slider = create_forecast_tab()
    ...
    # 啟動時的初始化載入
    demo.load(fn=create_dashboard, inputs=[view_radio, month_filter], outputs=[plot_output, plot_peak_output, table_output])
